...
$(function () {
  ...
  //   .direction('n')
  //   .html(function(d) {
  //     return "test"//nameById.get(d.id) + "<br/>Income change: " + (rateById.get(d.id)*100).toFixed(2) + "%" +
  ...
  function loaded(error, results) {
    ...
    airports = svg.append("g")
      .attr("class", "airports")
      .selectAll("path")
      .data(topojson.feature(airportsJson, airportsJson.objects.collection).features)
      .enter()
      .append("path")
  ...
  //  launchPlannedAnimation(fligthsJson.features);
    ...
    function clickState(d) {
      ...
      planes.selectAll("path")
          .classed("active", centered && function(d) { return d === centered; });
      countries.selectAll("path")
          .classed("active", centered && function(d) { return d === centered; });      
      airports.selectAll("path")
          .classed("active", centered && function(d) { return d === centered; });
  ...
  function removeAllFligths() {
    planes.selectAll("path").remove();
    routes.selectAll("path").remove();
  }
  function transition(plane, route, fligth) {
    ...
    plane.transition()
      ...
      .remove();
      console.log("transition: "+JSON.stringify(fligth.properties));
  }
  ...
  function executeFlightPlan(fligthPlanFeature) {
    ...
    var route = routes.append("path").datum(fligthPlanFeature.geometry).attr("class", "route").attr("d", path);
    var plane = planes.append("path").attr("class", "plane")
    ...
    transition(plane, route,fligthPlanFeature);
  }
  ...
  window.stopFlightAnimations = function(){
    removeAllFligths();
  }
